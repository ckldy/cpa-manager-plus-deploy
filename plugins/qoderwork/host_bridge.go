...
type hostHTTPStream struct {
	...
	// A host bridge can accept do_stream then fail its first stream_read with a
	// transport-level error. Retrying from the beginning is safe only before
	// any bytes reached the consumer; retain the original request bytes solely
	// for that one bounded recovery path.
...
func hostHTTPDoStream(req *http.Request) (*hostHTTPStream, int, http.Header, error) {
	...
	if req.Header.Get("X-Qoder-Direct-Stream") == "1" {
		req.Header.Del("X-Qoder-Direct-Stream")
	...
	return &hostHTTPStream{
		...
		fallbackReq:  req,
		fallbackBody: append([]byte(nil), bodyBytes...),
...
func hostHTTPDoStreamDirect(req *http.Request, bodyBytes []byte) (*hostHTTPStream, int, http.Header, error) {
	...
	newReq.Header = req.Header.Clone()
	// The Qoder gateway aborts HTTP/2 streams for qmodel_preview on this
	// route. Direct recovery therefore pins the fallback transport to HTTP/1.1.
	client := &http.Client{Timeout: 120 * time.Second, Transport: &http.Transport{
		MaxIdleConns: 20, IdleConnTimeout: 90 * time.Second, MaxIdleConnsPerHost: 5,
...
func (s *hostHTTPStream) Read() ([]byte, bool, error) {
	...
	if err != nil {
		return s.retryDirectBeforeDelivery(err)
	}
	...
	if resp.Error != "" {
		return s.retryDirectBeforeDelivery(fmt.Errorf("%s", resp.Error))
	}
...
// retryDirectBeforeDelivery is a narrowly-scoped recovery for a host bridge
// failure before it produced any bytes. It makes at most one fresh request;
// after any delivered byte, retrying would duplicate SSE output and is unsafe.
func (s *hostHTTPStream) retryDirectBeforeDelivery(bridgeErr error) ([]byte, bool, error) {
	if s.delivered || s.fallbackUsed || s.fallbackReq == nil {
		return nil, true, bridgeErr
	}
	s.fallbackUsed = true
	direct, statusCode, _, err := hostHTTPDoStreamDirect(s.fallbackReq, s.fallbackBody)
	if err != nil {
		return nil, true, fmt.Errorf("host stream read: %v; direct recovery: %w", bridgeErr, err)
	}
	if statusCode >= 400 {
		return nil, true, fmt.Errorf("host stream read: %v; direct recovery upstream %d", bridgeErr, statusCode)
	}
