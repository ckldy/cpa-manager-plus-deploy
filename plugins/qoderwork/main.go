...
func handleExecExecute(raw []byte) ([]byte, error) {
	...
	// The host stream bridge enforces a ~60s per-stream deadline; large agent
	// requests legitimately need longer. Route the whole non-streaming
	// executor path through the plugin's own bounded 120s direct client.
	httpReq.Header.Set("X-Qoder-Direct-Stream", "1")
