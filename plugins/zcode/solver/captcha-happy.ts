...
(function() {
  // Realm-global stubs that real browsers expose but happy-dom's VM realm
  // does not resolve as bare identifiers (pe.XX / FeiLin bytecode calls the
  // print/stop functions with no receiver). In Bun the global window alias
  // exposes them on globalThis; in Node we must add them inside the window
  // realm itself, otherwise the risk engine throws print-is-not-defined
  // and the solve stalls before verification completes.
  try {
    if (typeof globalThis !== "undefined") {
      ...
      if (typeof globalThis.moveTo !== "function") globalThis.moveTo = function() {};
      if (typeof globalThis.moveBy !== "function") globalThis.moveBy = function() {};
      if (typeof globalThis.showModalDialog !== "function") globalThis.showModalDialog = function() { return null; };
...
async function createDom(region, prefix) {
  ...
  const needsGlobalAlias =
    typeof Bun !== "undefined" ||
    process.env.CAPTCHA_FORCE_GLOBAL_ALIAS === "1";
  if (needsGlobalAlias) {
    const g = globalThis;
