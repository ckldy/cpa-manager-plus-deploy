...
func TestConfiguredModelsBypassCatalogCacheAndUseModelsDevMetadata(t *testing.T) {
	...
	if len(response.Models) != 2 || response.Models[0].ID != "workbuddy-serve-second" || response.Models[0].Name != "Second configured" || response.Models[0].ContextLength != 2222 || response.Models[1].ID != "workbuddy-serve-first" || response.Models[1].Name != "First configured" || response.Models[1].ContextLength != 1111 {
