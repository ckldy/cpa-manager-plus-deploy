...
// modelPublicPrefix marks WorkBuddy-provided models in the host model list so
// they stay distinguishable from other providers' identically-named models.
// The host forwards the client-facing model string to the executor unchanged,
// so resolveUpstreamModel strips the prefix again before the upstream call.
// Internal snapshots, caches and the panel keep bare upstream IDs; only
// ModelProvider responses are prefixed.
const modelPublicPrefix = "workbuddy-"

// prefixModelIDs returns a fresh slice with modelPublicPrefix applied to every
// model ID (idempotent). The input slice is never mutated — it may be the
// readiness snapshot's or cache's own backing array.
func prefixModelIDs(models []pluginapi.ModelInfo) []pluginapi.ModelInfo {
	out := make([]pluginapi.ModelInfo, len(models))
	for i, m := range models {
		if !strings.HasPrefix(m.ID, modelPublicPrefix) {
			m.ID = modelPublicPrefix + m.ID
		}
		out[i] = m
	}
	return out
...
func resolveUpstreamModel(model string, attributes map[string]string) string {
	...
	// Strip the public display prefix first: clients request the prefixed
	// public ID while aliases and upstream IDs are bare.
	if strings.HasPrefix(m, modelPublicPrefix) {
		m = strings.TrimPrefix(m, modelPublicPrefix)
	}
...
func handleModelStatic(raw []byte) ([]byte, error) {
	...
	return okEnvelope(pluginapi.ModelResponse{Provider: providerName, Models: prefixModelIDs(models)})
}
